...
import {createRouter, OpenRouteOptions, Router, RouterOptions} from './router';
...
export function createClientRouter<M = any, D = any>({...options}: ClientRouterOptions = {}): Router<M, D> {

    const router = createRouter<M>(options);
